...
public class RAGChatTest {
  ...
  /**
   * A simple assistant with no funky stuff like memory and the like. Only a chat method to get a
   * response for a user message.
   */
  interface ChatBot {
  ...
  public void testThat_ragWorks() throws JsonProcessingException {
    ...
    ChatBot assistant = AiServices.builder(ChatBot.class).chatLanguageModel(model).chatMemory(
        MessageWindowChatMemory.withMaxMessages(10)).contentRetriever(
            EmbeddingStoreContentRetriever.from(embeddingStore)).build();
  ...
  public void testThat_ragWorks_lowLevel() {
    ...
    assertTrue(answer.toLowerCase().contains("slow"), "Not slow");
    assertTrue(answer.toLowerCase().contains("dog") || answer.toLowerCase().contains("golden retriever"), "Not a dog");
  }

  /**
   * Advanced RAG test where also metadata (file name) will be appended to the answer.
   */
  @Test
  public void testThat_ragWorks_advanced() {
    ...
    RagAssistant assistant = AiServices.builder(RagAssistant.class).chatLanguageModel(model).retrievalAugmentor(
        retrievalAugmentor).chatMemory(MessageWindowChatMemory.withMaxMessages(10)).build();
